...
- Corpo a **colonna singola**: i blocchi descrittivi come righe `label / valore` (`.row`), l'organigramma e le schede persona in `.main`; a seguire la sezione evidenziata `.rail` "Per Corley" a tutta larghezza con ICP fit / Pain / Leve: la lettura dalle nostre angolazioni (AWS, innovazione tecnologica). Niente materiale di ingaggio: il dossier è la fotografia del cliente, le mosse le decide il sales. Il layout a due colonne è stato abbandonato: senza la parte di ingaggio non c'è più una "rail operativa" da tenere a fianco, e la colonna unica legge meglio sia a schermo sia in PDF.
...
Il blocco `<style>` di riferimento include già le regole `@media print` necessarie (formato A4, `print-color-adjust:exact` per mantenere i colori dei badge, `break-inside:avoid` su organigramma/gap/schede persona, e il ripristino della striscia fatti a 3 colonne: la media query responsive da 760px scatta anche in stampa A4). Mantienile invariate: sono ciò che fa sì che il PDF conservi la stessa leggibilità dell'HTML.

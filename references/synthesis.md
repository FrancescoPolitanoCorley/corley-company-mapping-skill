...
5. Deriva la parte azionabile (è il valore del dossier, non un riempitivo):
   ...
   - **Mossa**: il contatto da approcciare per primo, canale e **obiettivo di chiusura della call** (con cosa si esce: es. un assessment proposto e una data fissata in call). La mossa è una proposta del dossier: non prescrivere chi scrive internamente (lo decide il team sales, anche se l'account manager è noto dai fogli lead) e **niente scadenze né riferimenti temporali relativi** ("entro venerdì", "questa settimana", "la finestra si raffredda"): il dossier può restare fermo per settimane e non deve invecchiare. Le date assolute sui fatti (es. l'evento del contatto) vanno bene. Una call senza exit definito produce una chiacchierata.
   - **Bozza apertura**: riga di **Oggetto** + 3-4 righe pronte da incollare, personalizzate sul contatto ★ (aggancio a gancio o trigger, una frase sul valore, una call to action leggera), marcate "da rivedere". Niente claim non verificabili ("è il lavoro che facciamo più spesso") e niente riferimenti temporali relativi ("la prossima settimana"): la bozza deve funzionare anche usata un mese dopo. Meglio dire cosa il destinatario ottiene dalla call.
